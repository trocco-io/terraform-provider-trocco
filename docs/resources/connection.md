...
- `auth_type` (String) Databricks: The Auth Type for the Databricks connection. It must be one of `pat` or `oauth-m2m`. Custom Connector: read-only, reflects the auth type (`api_key` or `oauth2`) of the referenced custom connector definition.
...
- `scopes` (List of String) Custom Connector: OAuth2 scopes. When specified on update, this fully replaces the existing scope list; omitting it keeps the scopes stored on the server. Computed because the server always returns a (possibly empty) list for an `oauth2` definition, which would otherwise conflict with an unset configuration.
