...
resource "trocco_notification_destination" "http" {
  type = "http"
  http_config = {
    name = "example-http-destination"
    url  = "https://example.com/hooks/datamart"
  }
}
...
resource "trocco_bigquery_datamart_definition" "test_bigquery_datamart_notifications" {
  ...
  notifications = [
    ...
    },
    {
      ...
      http_notification_destination_id = trocco_notification_destination.http.id
      notification_type                = "job"
      notify_when                      = "failed"
      message                          = "{\"text\": \"datamart job failed\"}"
    },
    {
      destination_type                 = "http"
      http_notification_destination_id = trocco_notification_destination.http.id
