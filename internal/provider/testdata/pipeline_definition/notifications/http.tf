...
resource "trocco_notification_destination" "http" {
  type = "http"
  http_config = {
    name = "pipeline-definition-acc-http"
  ...
  notifications = [
    {
      ...
      http_config = {
        notification_id = trocco_notification_destination.http.id
        message         = "{\"text\": \"workflow failed\"}"
      }
    },
    {
      type             = "job_time_alert"
      ...
      http_config = {
        notification_id = trocco_notification_destination.http.id
        message         = "{\"text\": \"workflow is taking longer than 5 minutes\"}"
      }
    },
  ]
