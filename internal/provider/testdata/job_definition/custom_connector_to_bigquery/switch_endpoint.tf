resource "trocco_custom_connector_input" "custom_connector_to_bigquery_test" {
  ...
  endpoints = [
    ...
    {
      # A second endpoint whose snapshot attributes all differ from the first
      # one's, so that pointing the job definition at it rebuilds the snapshot
      # with visibly different values. It declares the same named values, which
      # is what lets the previously configured ones survive the rebuild.
      name                = "list_users_v2"
      path                = "/v2/users"
      method              = "GET"
      jsonpath_root       = "$.data"
...
resource "trocco_job_definition" "custom_connector_to_bigquery" {
  name                     = "Custom Connector to BigQuery Test (Switched)"
  description              = "Test job definition pointed at another endpoint of the same custom connector definition"
  ...
  input_option = {
    custom_connector_input_option = {
      # Pointed at the second endpoint of the same definition. The API
      # rebuilds the snapshot from the newly referenced endpoint while
      # updating this job definition, so the plan must not pin the snapshot
      # attributes to their prior values.
      custom_connector_endpoint_id   = trocco_custom_connector_input.custom_connector_to_bigquery_test.endpoints[1].id
