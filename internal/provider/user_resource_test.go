...
func TestAccUserResourceAutoGeneratedPassword(t *testing.T) {
	resourceName := "trocco_user.test"
	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// Create and Read testing with auto-generated password
			{
				Config: providerConfig + LoadTextFile("testdata/user/auto_generated_password.tf"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "email", "test@example.com"),
					resource.TestCheckResourceAttr(resourceName, "role", "admin"),
					resource.TestCheckResourceAttrSet(resourceName, "id"),
...
func TestAccUserResourceInvalidPassword(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "auto_generated_with_password",
			configFile:  "testdata/user/auto_generated_with_password.tf",
			expectError: "When password_auto_generated is set to true, password cannot be specified",
		},
	}
