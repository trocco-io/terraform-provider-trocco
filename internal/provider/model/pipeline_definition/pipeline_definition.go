...
func (m *PipelineDefinition) ToCreateInput() *client.CreatePipelineDefinitionInput {
	...
	tasks := []pdp.Task{}
	if !m.Tasks.IsNull() {
		m.Tasks.ElementsAs(context.Background(), &tasks, false)
	}
...
func (m *PipelineDefinition) ToUpdateWorkflowInput(state *PipelineDefinition) *client.UpdatePipelineDefinitionInput {
	...
	if !state.Tasks.IsNull() {
		var stateTasks []*Task
		state.Tasks.ElementsAs(context.Background(), &stateTasks, false)
	...
	if !m.Tasks.IsNull() {
		var modelTasks []*Task
		m.Tasks.ElementsAs(context.Background(), &modelTasks, false)
