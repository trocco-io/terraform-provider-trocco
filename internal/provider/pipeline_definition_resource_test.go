		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "tasks.0.bigquery_data_check_config.ignore_result", "false"),
				),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "tasks.0.snowflake_data_check_config.ignore_result", "false"),
				),
			},
			// Import testing
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "tasks.0.redshift_data_check_config.ignore_result", "false"),
				),
			},
			// Import testing
			{
...
func TestAccPipelineDefinitionResourceForDataCheckDatabricks(t *testing.T) {
	resourceName := "trocco_pipeline_definition.databricks_data_check_query_check"
	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config:      providerConfig + LoadTextFile("testdata/pipeline_definition/databricks_data_check/create.tf"),
				ExpectError: nil,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", "databricks_data_check"),
					resource.TestCheckResourceAttr(resourceName, "tasks.0.databricks_data_check_config.query", "          SELECT COUNT(*) FROM examples\n"),
					resource.TestCheckResourceAttr(resourceName, "tasks.0.databricks_data_check_config.ignore_result", "false"),
				),
			},
			{
				Config:      providerConfig + LoadTextFile("testdata/pipeline_definition/databricks_data_check/update.tf"),
				ExpectError: nil,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "tasks.0.databricks_data_check_config.ignore_result", "true"),
				),
			},
			// Import testing
			{
				...
				ImportStateVerifyIgnore: []string{
					...
					"tasks.0.key",
					// INFO: The `query` attribute is trimmed and set in state, so different from the resource config.
					"tasks.0.databricks_data_check_config.query",
				},
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					pipelineDefinitionID := s.RootModule().Resources[resourceName].Primary.ID
					return pipelineDefinitionID, nil
