## 0.19.0
BREAKING CHANGES:
- Changed column types from `List` to `Set` for order-independent collections across multiple resources:
  - `trocco_job_definition` resource:
    - `snowflake_output_option_merge_keys`
    - Various filter-related fields (`filter_columns`, etc.)
  - `trocco_pipeline_definition` resource:
    - `variables`
    - `notifications`
    - `schedules`
    - `labels`
    - Various task configuration fields
  - `trocco_bigquery_datamart_definition`, `trocco_resource_group`, and `trocco_team` resources
  - This change improves Terraform state management and prevents unnecessary diffs due to element ordering

FIXES:
- Added proper error handling for HTTP response body close operations
- Fixed validation logic in Snowflake and PostgreSQL input/output options

CHORE:
- Updated golangci-lint to v2.2.0 with enhanced linting rules (bodyclose, contextcheck, errorlint, gocritic, gosec, etc.)
- Updated Terraform plugin dependencies to latest versions
- Improved context handling throughout the codebase by passing context explicitly
- Enhanced test structure and maintainability by separating test files into testdata directory
- Removed duplicate testdata file
- Enforced lowercase convention for log messages
